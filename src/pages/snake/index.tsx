...
const equateCells = (cell1: Cell, cell2: Cell): boolean =>
...
export const SnakeGamePage = () => {
  ...
  const [head, setHeadPiece] = useState<Piece>({
    x: 10,
    y: 10,
    body: [
      { x: 10, y: 9 },
  ...
  const getEmptyCell = (): Cell => {
    while (true) {
      ...
      if (
        ![...obstacleCells, ...head.body, head, foodCell].find((cell) =>
          equateCells(cell, emptyCell)
        ) &&
        !rowBoarders.includes(emptyCell.x) &&
        !columnBoarders.includes(emptyCell.y)
      ) {
        return emptyCell;
      }
    }
  ...
  const onMove = () => {
    if (equateCells(head, foodCell)) {
      ...
      rowBoarders.includes(head.x) ||
      columnBoarders.includes(head.y)
    ) {
      // Collision with wall
      setPlaying(false);
    ...
    <main className="container">
      <section className="control-section row mb-5">
        <div className="col-12 justify-content-evenly">
          <button className="btn btn-secondary mx-1" onClick={onExit}>
            Quit
          </button>
          <button className="btn btn-secondary mx-1" onClick={onSettings}>
            Settings
          </button>
          <button className="btn btn-secondary mx-1" onClick={onLeaderboard}>
            Leaderboard
          </button>
          <button className="btn btn-secondary mx-1" onClick={onUpload}>
            Upload
          </button>
          <button className="btn btn-secondary mx-1" onClick={onDownload}>
            Download
          </button>
          <button className="btn btn-secondary mx-1" onClick={onRestart}>
            Restart
          </button>
          <button
              ...
              {grid.map((row, rowIndex) => (
                <tr key={rowIndex}>
                  {row.map((cell, columnIndex) => (
                    <td title={`(${cell.x}, ${cell.y})`} key={columnIndex}>
                      {getCharacter(cell)}
                    </td>
                  ))}
                </tr>
